...
// Every query below is scoped to this user's id, taken from the
// session (never from $_GET/$_POST), so nobody can act on another
// user's data no matter what id they put in the URL.
$userId = current_user_id();

// ---------------- NOTES: DELETE (soft) ----------------
if (isset($_GET['delete'])) {
    $sno = (int) $_GET['delete'];
    $stmt = mysqli_prepare($conn, "UPDATE `notes` SET `deleted_at` = NOW() WHERE `sno` = ? AND `user_id` = ?");
    ...
    if (mysqli_stmt_affected_rows($stmt) > 0) {
        ...
        // same message either way, so we don't leak which one it is.
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['snoEdit'])) {
        ...
        if ($title === '' || $description === '') {
            ...
            $stmt = mysqli_prepare($conn, "UPDATE `notes` SET `title` = ?, `description` = ? WHERE `sno` = ? AND `user_id` = ?");
            mysqli_stmt_bind_param($stmt, "ssii", $title, $description, $sno, $userId);
            mysqli_stmt_execute($stmt);
            if (mysqli_stmt_affected_rows($stmt) > 0) {
                $_SESSION['flash'] = ['type' => 'success', 'text' => '<strong>Success!</strong> Note updated successfully.'];
            } else {
                $_SESSION['flash'] = ['type' => 'warning', 'text' => 'That note could not be found, or nothing changed.'];
            }
            mysqli_stmt_close($stmt);
        }
        header("Location: index.php");
    ...
    if (isset($_POST['title']) && isset($_POST['desc'])) {
        ...
        if ($title === '' || $description === '') {
            ...
            $stmt = mysqli_prepare($conn, "INSERT INTO `notes` (`user_id`, `title`, `description`) VALUES (?, ?, ?)");
    ...
    if (isset($_POST['reminder_title'])) {
        ...
        if ($title === '' || $date === '' || $time === '') {
            ...
            $stmt = mysqli_prepare($conn, "INSERT INTO `reminders` (`user_id`, `title`, `description`, `remind_date`, `remind_time`, `repeat_option`, `custom_days`) VALUES (?, ?, ?, ?, ?, ?, ?)");
            mysqli_stmt_bind_param($stmt, "isssssi", $userId, $title, $desc, $date, $time, $repeat, $custom);
            if (mysqli_stmt_execute($stmt)) {
...
if (isset($_GET['delete_reminder'])) {
    ...
    $stmt = mysqli_prepare($conn, "UPDATE `reminders` SET `deleted_at` = NOW() WHERE `id` = ? AND `user_id` = ?");
...
if (isset($_GET['toggle_reminder'])) {
    ...
    $stmt = mysqli_prepare($conn, "UPDATE `reminders` SET `is_done` = 1 - `is_done` WHERE `id` = ? AND `user_id` = ?");
    mysqli_stmt_bind_param($stmt, "ii", $id, $userId);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
    header("Location: index.php#reminders-section");
...
$reminders = [];
$stmt = mysqli_prepare($conn, "SELECT * FROM `reminders` WHERE `deleted_at` IS NULL AND `user_id` = ? ORDER BY `remind_date` ASC, `remind_time` ASC");
...
?>
  ...
  <div class="container my-5" id="notes-section">
    ...
    <table class="table table-striped table-hover border align-middle">
      ...
      <tbody>
        <?php
        $notesStmt = mysqli_prepare($conn, "SELECT * FROM `notes` WHERE `deleted_at` IS NULL AND `user_id` = ? ORDER BY `sno` ASC");
        mysqli_stmt_bind_param($notesStmt, "i", $userId);
        mysqli_stmt_execute($notesStmt);
        $result = mysqli_stmt_get_result($notesStmt);
        $displayNum = 0;
